...
function KCT_GoogleLocationClass() {
    ...
    function GetCountryCityZoneAddress(results) {
        ...
        //if (Intern_PhysicalAddElem !== undefined && Intern_PhysicalAddElem !== null && Intern_PhysicalAddElem !== "")
            modifyPhysicalAdd = false;
            Intern_PhysicalAddElem.val(results[0].formatted_address);

        for (var i = 0; i < results[0].address_components.length; i++) {
            for (var b = 0; b < results[0].address_components[i].types.length; b++) {
                //there are different types that might hold a city admin_area_lvl_1 usually does in come cases looking for sublocality type will be more appropriate
                if (results[0].address_components[i].types[b].indexOf("locality") > -1) {
                    ZoneNamesArr[ZoneNamesArr.length] = results[0].address_components[i].long_name;
                    break;
                }
        ...
        if (Intern_CountryElem !== undefined && Intern_CountryElem !== null && Intern_CountryElem !== "") {
            ...
            if (cityId !== null) {
                ...
                $.each(ZoneDropDownDict, function (index, value) {
                    if (zoneId === null)
                        for (var i = 0; i < ZoneNamesArr.length; i++)
    ...
    function codeLatLng(lat, lng) {
        ...
        alert("Drawing the map, forth in 'codeLatLng'");
        return;
    ...
    function ShowMap_LatLong(lat, lon, zoom, posAlreadySpec) {
        ...
        if (Intern_Map) {
            ...
            //});
            //marker.setMap(Intern_Map);
            //SetDragDragEnd(marker);
            ...
            Intern_MapMarker.setMap(Intern_Map);
            SetDragDragEnd(Intern_MapMarker);
        }
    }
    ...
    function ShowMap_LocationString(str, zoom) {
        ...
        geoCoder.geocode(request, function (results, status) {
            if (status === google.maps.GeocoderStatus.OK) {
                ...
                alert("Drawing the map, once in 'ShowMap_LocationString'");

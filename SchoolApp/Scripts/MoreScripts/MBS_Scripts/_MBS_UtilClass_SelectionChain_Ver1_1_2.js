...
function MBS_SelectionChainClass(nameOfChain, MaxSize) {
    ...
    var Intern_GivenDataStore = null;
    ...
    function SetChangeHandlers() {
        ...
        if (Intern_UseLocalStore === true) {
            var LocalDataStore;
            if (Intern_GivenDataStore !== null)
                LocalDataStore = Intern_GivenDataStore;
            else
                LocalDataStore = new KCT_LocalStorageClass(Intern_NameOfChain, Intern_LanguageID, Intern_LatestUpdateDate, Intern_RenewLocationDataAfter, Intern_HTMLLocalStore_MaxSize);
        }
                ...
                if (Intern_ArrElements[a].val() === Intern_ArrDefaultVals[a]) {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            ...
                            var ret = Utility_ManageJsonObject(result);

                            Intern_ArrElements[a + 1].removeAttr("disabled").html(Intern_OptionGenerFunct_Multiple(ret, Intern_ArrDefaultVals[a + 1], Intern_ArrDefaultText[a + 1]));
                            Intern_ArrElements[a + 1].val(Intern_ArrInitialVals[a + 1]);
                            Intern_ArrInitialVals[a + 1] = Intern_ArrDefaultVals[a + 1];
                            Intern_ArrElements[a + 1].change();
                            if (Intern_ChainEditLevel >= (a + 1)) Intern_ArrElements[a + 1].attr('disabled', 'disabled');                            
                            if (Intern_UseLocalStore === true) {
                                LocalDataStore.SetDataToLocalStore(Intern_ArrElemIDs[a] + "_" + Intern_ArrElements[a].val(), ret);
                            }
                        }
                    });
                }
    ...
    function setGivenDataStore(obj) {
        Intern_GivenDataStore = obj;
    }
    ...
    this.setGivenDataStore = setGivenDataStore;

...
function MBS_SelectionChainClass(nameOfChain, MaxSize) {
    ...
    function SetChangeHandlers() {
        ...
        for (var j = numElements - 1; j > 0; j--) {
            //Intern_ArrElements[j].html(Utilities_GenerateSelectOpt(Intern_ArrDefaultVals[j], Intern_ArrDefaultText[0])).attr('disabled', 'disabled');

            // Corrected by Kevin
            Intern_ArrElements[j].html(Utilities_GenerateSelectOpt(Intern_ArrDefaultVals[j], Intern_ArrDefaultText[j - 1])).attr('disabled', 'disabled');
        }
        ...
        for (var i = 0; i < numElements - 1; i++) {
            ChangeHandler = function () {
                ...
                if (Intern_ArrElements[a].val() === Intern_ArrDefaultVals[a]) {
                    //Intern_ArrElements[a + 1].html(Utilities_GenerateSelectOpt(Intern_ArrDefaultVals[j], Intern_ArrDefaultText[a])).attr('disabled', 'disabled');
                    for (var j = numElements - 1; j >= a + 1; j--) {
                        Intern_ArrElements[j].html(Utilities_GenerateSelectOpt(Intern_ArrDefaultVals[j], Intern_ArrDefaultText[a])).attr('disabled', 'disabled');
                    }
                }
                else {
                    for (var j = numElements - 1; j > a + 1; j--) {
                        Intern_ArrElements[j].html(Utilities_GenerateSelectOpt(Intern_ArrDefaultVals[j], Intern_ArrDefaultText[a + 1])).attr('disabled', 'disabled');
                    }
                    ...
                    if (Intern_UseLocalStore === true) {
                        ...
                        if (dataFromLocalStore !== undefined && dataFromLocalStore !== null) {
                            Intern_ArrElements[a + 1].removeAttr("disabled").html(Utilities_DictionaryToSelectOpt(dataFromLocalStore, Intern_ArrDefaultVals[a + 1], Intern_ArrDefaultText[a + 1]));
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            Intern_ArrElements[a + 1].removeAttr("disabled").html(Utilities_DictionaryToSelectOpt(result, Intern_ArrDefaultVals[a + 1], Intern_ArrDefaultText[a + 1]));
                            Intern_ArrElements[a + 1].val(Intern_ArrInitialVals[a + 1]);
                            Intern_ArrInitialVals[a + 1] = Intern_ArrDefaultVals[a + 1];
                            Intern_ArrElements[a + 1].change();
                            if (Intern_ChainEditLevel >= (a + 1)) Intern_ArrElements[a + 1].attr('disabled', 'disabled');                            
    ...
    //this.setArrElements = setArrElements;
